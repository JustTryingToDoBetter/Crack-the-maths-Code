...
export default function Buy(){
  ...
  const fields:[{key:keyof typeof form,label:string,type?:string}][0][]=[
    {key:"name",label:"Full name"}, {key:"email",label:"Email",type:"email"},
    {key:"phone",label:"Phone"}, {key:"address",label:"Delivery address"},
    {key:"city",label:"City"}, {key:"code",label:"Postal code"}
  ] as any;
  return (
    <div className="mx-auto max-w-6xl px-6 pt-12 pb-24">
      <h1 className="text-3xl font-bold">Checkout</h1>
      <div className="mt-8 grid gap-8 md:grid-cols-[1.3fr_.7fr]">
        <form method="POST" action="/api/payfast/checkout" className="rounded-3xl border border-white/10 bg-white/5 p-6">
          <div className="grid gap-4">
            {fields.map(({key,label,type}:any)=>(
              <label key={key} className="grid gap-2">
                <span className="text-sm text-white/80">{label}</span>
                <input name={key} required type={type||"text"} onChange={e=>setForm({...form,[key]:e.target.value})}
          ...
          <p className="mt-3 text-xs text-white/60">Payments processed securely. You'll receive email confirmation immediately.</p>
        </form>
        <aside className="rounded-3xl border border-white/10 bg-white/5 p-6 h-fit">
          ...
          <div className="mt-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between font-semibold text-white"><span>Total</span><span>R{(Number(total)+69).toFixed(2)}</span></div>
          </div>
          <div className="mt-6 text-xs text-white/70">• Secure card / Instant EFT / QR wallets<br/>• 30-day money-back guarantee</div>
        </aside>
      </div>
    </div>
  );
