...
export async function POST(req: NextRequest){
  ...
  const amount = (Number(process.env.NEXT_PUBLIC_PRODUCT_PRICE_CENTS)/100 || 299).toFixed(2);
  const pfParams: Record<string,string> = {
    ...
    amount, item_name: process.env.NEXT_PUBLIC_PRODUCT_NAME || "Crack the Maths Code",
    name_first: String(form.get("name")||""), email_address: String(form.get("email")||""),
    m_payment_id: `ord_${Date.now()}`
  };
  const signature = sign(pfParams);
  const html = `<html><body onload="document.forms[0].submit()">
